...
# For strict key validation, define allowed keys per top-level document.
ALLOWED_KEYS = {
    "npc.yaml": {"npc_version", "id", "display_name", "files", "data"},
    "identity.yaml": {"archetype", "description", "core_values", "purpose"},
    "persona.yaml": {"tone", "style", "verbosity", "humor", "speech_rules", "taboos", "example_lines"},
    "policy.yaml": {"boundaries", "refusal_policy", "truthfulness"},
    "memory.yaml": {"policy", "stores", "seeds"},
...
def load_yaml(path: Path) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    """
    Returns (data, error_message). Error message is None when successful.
    """
    try:
...
def validate_npc_dir(npc_dir: str | Path, *, strict: bool = True) -> ValidationReport:
    """
    Validate an NPC directory (e.g. npc/kevin.npc).

    strict=True:
      - flags unknown keys in YAML files
      - flags missing optional-but-expected structure details more aggressively
    """
    ...
    # Resolve file paths from manifest and validate presence
    resolved = {}
    ...
    if isinstance(db_rel, str) and db_rel.strip():
        ...
        # DB can be missing at first run — your call. We'll warn, not fail.
        if not db_path.exists():
            report.add("DB_MISSING", "Database file does not exist yet (ok for first run).", str(db_path))
    else:
        report.add("MANIFEST_TYPE", "data.db must be a non-empty string path.", _keypath("npc.yaml", "data", "db"))

    # Validate each referenced YAML
    _validate_identity(report, resolved.get("identity"))
    _validate_persona(report, resolved.get("persona"))
    _validate_policy(report, resolved.get("policy"))
    _validate_memory(report, resolved.get("memory"))
    _validate_kernel(report, resolved.get("kernel"))
    _validate_actions(report, resolved.get("actions"))
    _validate_tools(report, resolved.get("tools"))

    # If strict, enforce no unknown keys across all docs
    if strict:
        for shortname, path in resolved.items():
            if not path:
                continue
            fname = path.name
            ...
            doc, err2 = load_yaml(path)
            if err2 or doc is None:
                continue
            _validate_allowed_keys(report, fname, doc, allowed)
...
def _ensure_file(report: ValidationReport, path: Optional[Path], label: str) -> Optional[Dict[str, Any]]:
    if path is None:
...
def _validate_identity(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "identity")
    if not doc:
        return
...
def _validate_persona(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "persona")
    if not doc:
        return
    ...
    if not isinstance(verbosity, str) or verbosity not in ALLOWED_VERBOSITY:
        report.add(
            "PERSONA_ENUM",
            f"verbosity must be one of {sorted(ALLOWED_VERBOSITY)}.",
            _keypath("persona.yaml", "verbosity"),
        )
...
def _validate_policy(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "policy")
    if not doc:
        return
...
def _validate_memory(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "memory")
    if not doc:
        return
    ...
    if not isinstance(privacy_mode, str) or privacy_mode not in ALLOWED_PRIVACY_MODE:
        report.add(
            "MEMORY_ENUM",
            f"policy.privacy_mode must be one of {sorted(ALLOWED_PRIVACY_MODE)}.",
            _keypath("memory.yaml", "policy", "privacy_mode"),
        )
...
def _validate_kernel(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "kernel")
    if not doc:
        return
...
def _validate_actions(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "actions")
    if not doc:
        return

    if not isinstance(doc.get("output_modes"), list) or not all(isinstance(x, str) for x in doc["output_modes"]):
...
def _validate_tools(report: ValidationReport, path: Optional[Path]) -> None:
    doc = _ensure_file(report, path, "tools")
    if not doc:
        return
    ...
    if not isinstance(registry, list):
        ...
        # v0.1: registry items can be strings or objects later; keep permissive but sane
...
def main() -> None:
    ...
    parser.add_argument("--non-strict", action="store_true", help="Allow unknown keys.")
    args = parser.parse_args()

    report = validate_npc_dir(args.npc_dir, strict=not args.non_strict)

    if report.ok:
        print(f"✅ OK: {report.npc_id} ({report.display_name}) v{report.npc_version}")
        sys.exit(0)

    print(f"❌ INVALID NPC: {args.npc_dir}")
