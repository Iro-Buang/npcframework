...
# NOTE: meta is allowed everywhere as the "degenerate nonsense bucket".
ALLOWED_KEYS = {
...
# Required keys per file (minimal)
REQUIRED_KEYS = {
...
def validate_npc_dir(npc_dir: str | Path, *, strict: bool = True) -> ValidationReport:
    ...
    # manifest: required + allowed keys
    ...
    # DB: warn-only if missing (first run)
    db_rel = data.get("db")
    if isinstance(db_rel, str) and db_rel.strip():
        db_path = npc_path / db_rel
        if not db_path.exists():
            # warning-but-fatal in current report model; if you want true warnings later, add severity.
    ...
    # Validate each referenced YAML file
...
def _ensure_file(
    ...
    *,
    ...
    # strict key checks (unknown keys) + meta validation
...
def _validate_persona(report: ValidationReport, path: Optional[Path], *, strict: bool) -> None:
    ...
    # Optional lists (validated if present)
...
def _validate_allowed_keys(report: ValidationReport, file_name: str, doc: Dict[str, Any], allowed: set[str]) -> None:
    ...
    - meta is always allowed (should already be in allowed, but this makes it bulletproof)
    """
    for k in doc.keys():
        if k == META_KEY:
            continue
